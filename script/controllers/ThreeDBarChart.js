  ...
  ThreeDBarChart.prototype = (_ThreeDBarChart$proto = {
    ...
    for (var _i2 = 0; _i2 < data.length; _i2++) {
      ...
      for (var j = 0; j < d3.values(data[_i2])[0].length; j++) {
        var bar = new ThreeDBar(this.gl, this.shaderProgram, this.mvMatrix, {
          height: yScale(d3.values(data[_i2])[0][j].Value),
          barColor: bc
        }).init();
    ...
    ;
    var axisX = new CoordinatePlaneText(this.gl, this.shaderProgram, this.mvMatrix, {
      x: -w / 2,
      y: -d,
      z: 0,
      width: w,
      height: d,
      xTicksCount: this.maxColumnCount,
      yTicksCount: this.rowCount,
      tickStep: d / (this.rowCount - 1),
      ledge: l,
      rotate: 90,
      xRotation: 1,
      text: {
    ...
    var axisY = new CoordinatePlaneText(this.gl, this.shaderProgram, this.mvMatrix, {
      x: -w / 2,
      y: -1,
      z: -l,
      width: w,
      height: 1 + yScale(maxValue),
      xTicksCount: this.maxColumnCount,
      yTicksCount: axisTicksCount,
      tickStep: tickStep,
      ledge: l,
      text: {
        text: axisYValue,
    ...
    var axisZ = new CoordinatePlaneText(this.gl, this.shaderProgram, this.mvMatrix, {
      x: -w / 2 - l,
      y: -1,
      z: 0,
      width: d,
      height: 1 + yScale(maxValue),
      xTicksCount: this.rowCount,
      yTicksCount: axisTicksCount,
      tickStep: tickStep,
      ledge: l,
      rotate: -90,
