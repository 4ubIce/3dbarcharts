...
define('ThreeDBar', ['ClassHelper'], function (ClassHelper) {
  function ThreeDBar(gl, shaderProgram, mvMatrix, config) {
    this.cfg = {
      x: 0,
      y: 0,
      width: 0.2,
  ...
  ThreeDBar.prototype = {
    ...
    initBuffers: function initBuffers(width, height) {
      var barColor = this.getBarColor();
      this.cubeVertexPositionBuffer = this.gl.createBuffer();
      this.gl.bindBuffer(this.gl.ARRAY_BUFFER, this.cubeVertexPositionBuffer);
      var vertices = [// Front face
      -width, -1.0, width, width, -1.0, width, width, height, width, -width, height, width, // Back face
      -width, -1.0, -width, -width, height, -width, width, height, -width, width, -1.0, -width, // Top face
      -width, height, -width, -width, height, width, width, height, width, width, height, -width, // Bottom face
      -width, -1.0, -width, width, -1.0, -width, width, -1.0, width, -width, -1.0, width, // Right face
      width, -1.0, -width, width, height, -width, width, height, width, width, -1.0, width, // Left face
      -width, -1.0, -width, -width, -1.0, width, -width, height, width, -width, height, -width];
    ...
    draw: function draw() {
      ...
      this.gl.disableVertexAttribArray(this.shaderProgram.textureCoordAttribute); //this.gl.vertexAttribPointer(this.shaderProgram.textureCoordAttribute, 1, this.gl.FLOAT, false, 0, 0);
